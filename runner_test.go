...
func TestRunnerEverythingOK(t *testing.T) {
	test(is.New(t), func(is is.I, db *mgo.Database) {
		...
		// make three jobs
		job1 := jobs.New("things")
		job1.Data["thing_id"] = thing1ID
		job2 := jobs.New("things")
		job2.Data["thing_id"] = thing2ID
		job3 := jobs.New("things")
		job3.Data["thing_id"] = thing3ID
		is.NoErr(jobs.Put(db.C("jobs"), job1, job2, job3))

		var things []bson.M
		runner := jobs.NewRunner("test-1", db.C("jobs"), "things", func(job *jobs.J) error {
...
func TestRunnerTypes(t *testing.T) {
	test(is.New(t), func(is is.I, db *mgo.Database) {
		...
		job3 := jobs.New("thing")
		job3.Data["thing_id"] = thing3ID
		job4 := jobs.New("anotherthing")
		job5 := jobs.New("anotherthing")
		job6 := jobs.New("anotherthing")
		is.NoErr(jobs.Put(db.C("jobs"), job1, job2, job3, job4, job5, job6))

		var things []bson.M
		runner := jobs.NewRunner("test-1", db.C("jobs"), "thing", func(job *jobs.J) error {
			var thing map[string]interface{}
			is.OK(job.Data["thing_id"])
			is.NoErr(db.C("things").FindId(job.Data["thing_id"].(bson.ObjectId)).One(&thing))
			things = append(things, thing)
			return nil
		})
		...
		for iter.Next(&job) {
			...
			} else if job.Kind == "anotherthing" {
				is.Equal(job.Status, jobs.StatusNew)
				is.Equal(len(job.Tries), 0)
			}
